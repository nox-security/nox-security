...
export default async function LocalServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="section"><div className="container"><SectionHeading eyebrow="Customer feedback" title="Advice, workmanship and local support"/><ReviewGrid limit={3}/></div></section>
    ...
    <ConversionPanel title={context.label} text="Share the property or site, existing system, known faults and the work required. NOX will guide the correct survey, service, repair or takeover route." primaryLabel={context.label} audience={page.audience} serviceCategory={page.serviceType} enquiryType={context.type} sourceLabel={`${page.slug}-final`}/>
  </>
}
