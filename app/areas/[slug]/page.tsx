...
export default async function AreaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="section"><div className="container split-grid"><div><SectionHeading eyebrow="Local system design" title={`Security surveys and installations around ${area.name}`} text={area.focus}/><p className="lead">Nearby coverage includes {area.nearby}.</p></div><aside className="dark-panel"><h3>Start with one clear enquiry</h3><p>Send the initial project details for a tailored quotation. The NOX team will arrange a survey when the property needs to be assessed first.</p><div className="button-row"><Link className="button button-light" href="/get-quote">Get a Fire & Security Quote</Link></div></aside></div></section>
